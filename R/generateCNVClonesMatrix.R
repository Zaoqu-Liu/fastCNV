...
generateCNVClonesMatrix <- function(seuratObj, healthyClusters = NULL, values = "scores", cnv_thresh = 0.15) {
  ...
  cnv_matrix <- as.matrix(seuratObj@meta.data[, which(names(seuratObj@meta.data) == "1.p_CNV"):which(names(seuratObj@meta.data) == "X.q_CNV")])
  ...
  if (values == "scores") {
    ...
    alt_matrix <- matrix(0,
      nrow = nrow(cnv_matrix_clusters), ncol = ncol(cnv_matrix_clusters),
