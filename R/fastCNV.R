...
fastCNV <- function(seuratObj,
  ...
  if (prepareCounts == TRUE & aggregFactor >= 1000) {
    ...
    for (i in 1:length(seuratObj)) {
      seuratObj[[i]] <- prepareCountsForCNVAnalysis(seuratObj[[i]],
  ...
  if (length(seuratObj) > 1) {
    seuratObj <- CNVAnalysis(seuratObj,
      referenceVar = referenceVar, referenceLabel = referenceLabel,
    ...
    seuratObj <- CNVAnalysis(seuratObj[[1]],
      referenceVar = referenceVar, referenceLabel = referenceLabel,
  ...
  if (getCNVClusters == TRUE) {
    ...
    if (length(seuratObj) == 1) {
      seuratObj <- CNVCluster(seuratObj,
        referenceVar = referenceVar, tumorLabel = tumorLabel, k = k_clusters, h = h_clusters, plotDendrogram = plotDendrogram,
  ...
  if (doPlot == TRUE) {
    ...
    if (length(seuratObj) > 1) {
      for (i in 1:length(seuratObj)) {
        if (Seurat::Project(seuratObj[[i]]) == "SeuratProject") {
          Seurat::Project(seuratObj[[i]]) <- paste0("Sample", i)
        }
        if ("cnv_clusters" %in% names(seuratObj[[i]]@meta.data)) {
          splitPlotOnVar <- "cnv_clusters"
        }
        plotCNVResults(seuratObj[[i]],
          referenceVar = referenceVar, splitPlotOnVar = splitPlotOnVar,
      ...
      if ("cnv_clusters" %in% names(seuratObj@meta.data)) {
        splitPlotOnVar <- "cnv_clusters"
      }
      plotCNVResults(
        seuratObj = seuratObj, referenceVar = referenceVar, splitPlotOnVar = splitPlotOnVar,
