...
Computing the CNV without a reference."))
  ...
  if (dim(Seurat::GetAssay(seuratObj, assay = assay))[1] < topNGenes) {
    topNGenes <- as.numeric(dim(Seurat::GetAssay(seuratObj, assay = assay))[1])
  }

  # Get counts data with error handling
  rawCounts <- tryCatch(
    as.matrix(Seurat::GetAssay(seuratObj, assay = assay)@counts),
    error = function(e) {
      stop("Failed to get counts data from assay '", assay, "'. Error: ", e$message)
    }
  )
  ...
  if (length(commonGenes) == 0) {
    stop("No common genes found between data and gene metadata. Check if you're using human data.")
  }
  ...
  for (arm in unique(geneMetadata2$chr_arm)) {
    ...
    if (length(genes_by_arm[[arm]]) < 200) {
      ...
      if (n_to_select > 0) {
        top_arm_genes <- remaining_genes[order(averageExpression[remaining_genes], decreasing = TRUE)[1:n_to_select]]
  ...
  # CRITICAL FIX: Only keep genes that exist in rawCounts
  final_selected_genes <- intersect(final_selected_genes, rownames(rawCounts))
  ...
  genomicScores <- sapply(genomicWindows, function(g) {
    if (length(g) == 1) {
      ...
      colMeans(normCounts[g, , drop = FALSE])  # Force matrix with drop = FALSE
    }
  })
  # Ensure genomicScores is always a matrix
  if (!is.matrix(genomicScores)) {
    genomicScores <- as.matrix(genomicScores)
  }
  rm(normCounts)
  invisible(gc())
  ...
  rawGenomicAssay <- tryCatch(
    Seurat::CreateAssayObject(data = t(as.matrix(genomicScores))),
    error = function(e) {
      methods::new("Assay", counts = matrix(), data = t(as.matrix(genomicScores)), scale.data = matrix())
    }
  )
  suppressWarnings({
    seuratObj[["rawGenomicScores"]] <- rawGenomicAssay
  })
  
  genomicAssay <- tryCatch(
    Seurat::CreateAssayObject(data = as.matrix(genomicScoresTrimmed)),
    error = function(e) {
      methods::new("Assay", counts = matrix(), data = as.matrix(genomicScoresTrimmed), scale.data = matrix())
    }
  )
  suppressWarnings({
  ...
  # Calculate cnv_fraction with dimension check
  if (is.matrix(genomicScoresTrimmed) && ncol(genomicScoresTrimmed) > 0) {
    seuratObj[["cnv_fraction"]] <- colMeans(abs(genomicScoresTrimmed) > 0)
  } else if (is.vector(genomicScoresTrimmed)) {
    seuratObj[["cnv_fraction"]] <- mean(abs(genomicScoresTrimmed) > 0)
  } else {
    seuratObj[["cnv_fraction"]] <- rep(0, ncol(seuratObj))
  }
