...
CNVCallingList <- function(seuratList,
  ...
  funGenomicScore <- function(normcounts, GW = genomicWindows) {
    res <- sapply(GW, function(g) {
      subset_data <- normcounts[g, , drop = FALSE]  # Force matrix
      if (length(g) == 1) {
        subset_data[1, ]  # Return as vector if only one gene
      } else {
        colMeans(subset_data)
      }
    })
    # Ensure result is matrix
    if (!is.matrix(res)) {
      res <- as.matrix(res)
    }
    res
  ...
  if (length(commonGenes) == 0) {
    stop("No common genes found across samples and gene metadata. Check if you're using human data.")
  }
  ...
  if (length(aveExpr) < topNGenes) {
    topNGenes <- length(aveExpr)
  }
  ...
  for (arm in unique(geneMetadata2$chr_arm)) {
    ...
    if (length(genes_by_arm[[arm]]) < 200) {
      ...
      # Fix: ensure we don't try to select more genes than available
      n_to_select <- min(200, length(remaining_genes))
      if (n_to_select > 0) {
        top_arm_genes <- remaining_genes[order(aveExpr[remaining_genes], decreasing = TRUE)[1:n_to_select]]
  ...
  # CRITICAL FIX: Only keep genes that exist in all samples
  final_selected_genes <- intersect(final_selected_genes, commonGenes)
  
  if (length(final_selected_genes) == 0) {
    stop("No genes found after filtering. Check your data and gene metadata.")
  }
    ...
    for (arm in chr_arms) {
      ...
      if (N > windowSize) {
        gw <- lapply(seq(iter + 1, N - iter, by = windowStep), function(i) {
          genesArm[(i - iter):(i + iter), "hgnc_symbol"] |>
            unlist() |>
            as.character()
        })
  ...
  for (i in 1:length(seuratList)) {
    # Ensure data is matrix format and transposed correctly
    rawScores <- LgenomicScores[[i]]
    trimmedScores <- LgenomicScoresTrimmed[[i]]
    
    # Ensure proper matrix format
    if (!is.matrix(rawScores)) {
      rawScores <- as.matrix(rawScores)
    }
    if (!is.matrix(trimmedScores)) {
      trimmedScores <- as.matrix(trimmedScores)
    }
    
    # Ensure correct orientation: rows = features, cols = cells
    n_cells <- ncol(seuratList[[i]])
    if (ncol(rawScores) != n_cells && nrow(rawScores) == n_cells) {
      rawScores <- t(rawScores)
    }
    if (ncol(trimmedScores) != n_cells && nrow(trimmedScores) == n_cells) {
      trimmedScores <- t(trimmedScores)
    }
    
    # Final check
    if (ncol(rawScores) != n_cells) {
      stop("Dimension mismatch in rawScores for sample ", i, ": expected ", n_cells, " cells, got ", ncol(rawScores))
    }
    if (ncol(trimmedScores) != n_cells) {
      stop("Dimension mismatch in trimmedScores for sample ", i, ": expected ", n_cells, " cells, got ", ncol(trimmedScores))
    }
    
    # Create assay objects with error handling
    rawGenomicAssay <- tryCatch(
      Seurat::CreateAssayObject(counts = rawScores),
      error = function(e) {
        methods::new("Assay", counts = rawScores, data = matrix(), scale.data = matrix())
      }
    )
    suppressWarnings({
      seuratList[[i]][["rawGenomicScores"]] <- rawGenomicAssay
    })
    
    genomicAssay <- tryCatch(
      Seurat::CreateAssayObject(counts = trimmedScores),
      error = function(e) {
        methods::new("Assay", counts = trimmedScores, data = matrix(), scale.data = matrix())
      }
    )
    suppressWarnings({
    ...
    # Calculate cnv_fraction
    seuratList[[i]][["cnv_fraction"]] <- colMeans(abs(trimmedScores) > 0)
  }
  invisible(gc())
  return(seuratList)
}
