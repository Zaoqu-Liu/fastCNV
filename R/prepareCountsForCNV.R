  ...
  if (is.null(referenceVar) || aggregateByVar == F) {
    if (!"seurat_clusters" %in% colnames(seuratObj[[]]) | reClusterSeurat) {
      if (!is.null(sampleName)) {
        message(crayon::black, paste0("Running Seurat normalization and clustering for sample ", sampleName, ". This could take some time."))
      } else {
        message(crayon::black, "Running Seurat normalization and clustering. This could take some time.")
      }
      # Use NormalizeData instead of SCTransform for better compatibility
      seuratObj <- Seurat::NormalizeData(seuratObj, assay = assay, verbose = F)
      seuratObj <- Seurat::FindVariableFeatures(seuratObj, assay = assay, verbose = F)
      seuratObj <- Seurat::ScaleData(seuratObj, assay = assay, verbose = F)
      seuratObj <- Seurat::RunPCA(seuratObj, assay = assay, verbose = F)
      ...
      if (!is.null(sampleName)) {
        message(crayon::black, paste0("Seurat normalization and clustering done for sample ", sampleName))
      } else {
        message(crayon::black, "Seurat normalization and clustering done.")
      }
    }

    countsMat <- as.matrix(Seurat::GetAssayData(seuratObj, assay = assay, slot = "counts"))
    ...
    # Create assay object with fallback for compatibility
    aggregAssay <- tryCatch(
      Seurat::CreateAssayObject(counts = countsMat),
      error = function(e) {
        # Fallback: create manually for Seurat compatibility
        methods::new("Assay", counts = countsMat, data = matrix(), scale.data = matrix())
      }
    )
    ...
    countsMat <- as.matrix(Seurat::GetAssayData(seuratObj, assay = Seurat::Assays(seuratObj)[1], slot = "counts"))
