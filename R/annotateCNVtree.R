...
.get_gains <- function(cnv_mat, thresh) {
  ...
  if (length(gain_events) == 0) {
    gain_events <- lapply(1:nrow(cnv_mat), function(i) {
      vector()
    })
  }
...
.get_losses <- function(cnv_mat, thresh) {
  ...
  if (length(loss_events) == 0) {
    loss_events <- lapply(1:nrow(cnv_mat), function(i) {
      vector()
    })
  }
...
get_ancestralEvents <- function(tree, cnv_mat, major_events) {
  ...
  cnv_combns$node <- sapply(1:nrow(cnv_combns), function(i) {
    intersect(
      Ancestors(tree, cnv_combns$V1_indices[i]),
      Ancestors(tree, cnv_combns$V2_indices[i])
    )[1]
  })
  cnv_parent_events <- cnv_combns |>
    ...
    reframe(all_events = if (length(.data$shared_events) > 1) {
      ...
      unlist(.data$shared_events)
    }) |>
    group_by(.data$node) |>
    mutate(
      all_events = paste0(.data$all_events, collapse = " "),
      label = NA
    ) |>
    distinct() |>
    ungroup()
