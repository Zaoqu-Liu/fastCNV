  ...
  if (is.null(referenceVar) || aggregateByVar == F) {
    if (!"seurat_clusters" %in% colnames(seuratObj[[]]) | reClusterSeurat) {
      if (!is.null(sampleName)) {
        message(crayon::black, paste0("Running Seurat SCTransform and clustering for sample ", sampleName, ". This could take some time."))
      } else {
        message(crayon::black, "Running Seurat SCTransform and clustering. This could take some time.")
      }
      ...
      if (!is.null(sampleName)) {
        message(crayon::black, paste0("Seurat SCTransform and clustering done for sample ", sampleName))
      } else {
        message(crayon::black, "Seurat SCTransform and clustering done.")
      }
    }
